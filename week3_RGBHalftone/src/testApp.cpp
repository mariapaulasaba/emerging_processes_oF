...
void testApp::draw(){
    ...
    for(float y = 0; y < grabber.getHeight(); y+=step){
        for(float x = 0; x < grabber.getWidth(); x+=step){
            ...
            pixelPosition.x = ofMap(x, 0, grabber.getWidth(), 0, ofGetWidth());
            pixelPosition.y = ofMap(y, 0, grabber.getHeight(), 0, ofGetHeight());
            
            //move in
            ofSetColor(c.r, 0, 0);
            ofCircle(pixelPosition + redDir * c.r * scale*.01, scale * c.r/255.0);
